...
export const Home: React.FC = () => {
  ...
  const handleDeleteExam = async (examId: string, examTitle: string) => {
    ...
    try {
      ...
      if (selectedLessonForExams) {
        setSelectedLessonForExams((prev) => {
          if (!prev) return null;
          const updated = (prev.exams || []).filter((e) => e.id !== examId);
          return {
            ...prev,
            exams: updated,
            exams_count: updated.length
          };
        });
      }
      await loadData();
  ...
  return (
    <div className="max-w-6xl mx-auto px-3.5 sm:px-6 py-6 sm:py-10">
      ...
      {/* ========================================================= */}
      {/* LESSONS IN RESPONSIVE GRID LAYOUT (LESSON INFO ONLY) */}
      {/* ========================================================= */}
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div
              key={i}
              className="bg-white border border-gray-200 rounded-3xl p-5 animate-pulse space-y-4 shadow-xs h-48 flex flex-col justify-between"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
          {filteredLessons.map((lesson, lessonIdx) => {
            ...
            return (
              <div
                key={lesson.id}
                onClick={() => setSelectedLessonForExams(lesson)}
              ...
              >
                <div>
                  ...
                  {/* Title */}
                  <h2
                    className="text-base font-bold text-gray-900 group-hover:text-black leading-snug line-clamp-2 mb-2"
                    title={lesson.title}
                  >
                    {lesson.title}
                  </h2>
                ...
                <div className="mt-5 pt-3.5 border-t border-gray-100 flex items-center justify-between">
                  ...
                  <span className="inline-flex items-center gap-1 text-xs font-bold text-gray-900 group-hover:translate-x-0.5 transition-transform">
                    <span>Xem đề</span>
      ...
      {/* ========================================================= */}
      {/* EXAMS LIST MODAL (WHEN A LESSON IS CLICKED) */}
      {/* ========================================================= */}
      {selectedLessonForExams && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-3xl max-w-xl w-full shadow-2xl border border-gray-200 overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-150">
            {/* Modal Header */}
            <div className="p-5 sm:p-6 border-b border-gray-100 flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                  <span className="w-6 h-6 rounded-lg bg-gray-900 text-white font-black text-xs flex items-center justify-center">
                    {selectedLessonForExams.order_index ?? 1}
                  </span>
                  ...
                  <span className="inline-flex items-center gap-1 text-xs font-semibold text-gray-700 bg-gray-100 px-2 py-0.5 rounded-md">
                    ...
                    <span>{(selectedLessonForExams.exams || []).length} đề thi</span>
                  </span>
                </div>

                <h3 className="text-lg sm:text-xl font-black text-gray-900 leading-snug">
                  {selectedLessonForExams.title}
                </h3>

                {selectedLessonForExams.description && (
              ...
              <button
                onClick={() => setSelectedLessonForExams(null)}
                className="p-1.5 text-gray-400 hover:text-gray-900 rounded-xl hover:bg-gray-100 transition-colors cursor-pointer flex-shrink-0"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            {/* Modal Body: Exams List */}
            <div className="p-5 sm:p-6 overflow-y-auto space-y-3 flex-1">
              {(selectedLessonForExams.exams || []).length > 0 ? (
                (selectedLessonForExams.exams || []).map((exam) => (
                  <div
                    key={exam.id}
                    className="bg-gray-50/80 hover:bg-gray-50 border border-gray-200/80 hover:border-gray-300 rounded-2xl p-4 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3 group"
                  >
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 self-end sm:self-auto">
                      ...
                      <Link
                        to={`/exam/${exam.id}`}
                        className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gray-900 text-white hover:bg-black text-xs sm:text-sm font-semibold transition-all shadow-xs active:scale-95"
                      >
                ...
                <div className="bg-gray-50 border border-dashed border-gray-200 rounded-2xl p-8 text-center">
                  ...
                  {user ? (
                    <Link
                      to={`/create?lessonId=${selectedLessonForExams.id}`}
                      className="inline-flex items-center gap-1.5 text-xs sm:text-sm font-semibold text-gray-900 hover:underline"
                    >
                      <PlusCircle className="w-4 h-4" />
                    ...
                    <span className="text-xs text-gray-400">
                      Nội dung đề thi đang được biên soạn...
                    </span>
                  )}
                </div>
              )}
            ...
            {/* Modal Footer */}
            <div className="p-4 sm:p-5 bg-gray-50/70 border-t border-gray-100 flex items-center justify-between gap-2">
              {user ? (
                <Link
                  to={`/create?lessonId=${selectedLessonForExams.id}`}
                  className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-800 hover:text-black hover:underline"
                >
